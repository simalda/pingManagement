...
createListOfAllPoints(dataFromServer){
    let allPoints = []
     dataFromServer.forEach((item) => {
        item["pingTimeArrray"].forEach((item) => {
            allPoints.push(new Point( moment(item[this.time]),  item[this.pingValue]));
        });})
        return allPoints
}
  ...
  data.forEach((item) => {
    ...
    const pointsAfterStart = item["pingTimeArrray"].filter(
      ...
      dataForPing.push(new Point(moment(item[this.time]),   item[this.pingValue]));
    });
    dataSets.push({
      label: item["name"],
      fill: false,
  ...
  if (dateFilter === datePeriod.AllDateOption) {
    ...
    return this.scaleByDate("day", "HH", startDate, dataSets);
  } else if (dateFilter === datePeriod.HourDateOption) {
